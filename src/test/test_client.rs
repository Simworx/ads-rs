...
use crate::commands::{
    AddNotifRequest, DelNotifRequest, ReadRequest, WriteReadRequest, WriteRequest,
};
use crate::test::{config_test_server, ServerOpts};
use crate::{client::Client, AmsAddr, AmsNetId, Device, Error, Source, Timeouts};

fn run_test(opts: ServerOpts, f: impl Fn(Arc<Device>)) {
    ...
    f(client
        .device(AmsAddr::new(AmsNetId::new(1, 2, 3, 4, 5, 6), 851))
        .unwrap());
}
...
fn test_garbage_packet() {
    run_test(
        ...
        |device| {
            ...
            assert!(matches!(
                err,
                Error::Reply(_, "invalid packet or unknown AMS command", _)
            ));
        },
    )
}
...
fn test_readwrite() {
    run_test(ServerOpts::default(), |device| {
        ...
        assert!(matches!(
            device.read_exact(0x4021, 0, &mut buf),
...
fn test_fileaccess() {
    use crate::file::*;
    run_test(ServerOpts::default(), |device| {
        assert!(File::open(device.clone(), "blub", 0).is_err());
        let mut file = File::open(device.clone(), "/etc/passwd", WRITE).unwrap();
        assert!(File::open(device.clone(), "/etc/passwd", 0).is_err());
        assert!(file.read(&mut [0; 4]).is_err());
        file.write_all(b"asdf").unwrap();
        file.flush().unwrap();
        drop(file);

        let mut file = File::open(device.clone(), "/etc/passwd", READ).unwrap();
...
fn test_symbolaccess() {
    use crate::symbol::*;
    run_test(ServerOpts::default(), |device| {
        assert!(Handle::new(device.clone(), "blub").is_err());
        let handle = Handle::new(device.clone(), "SYMBOL").unwrap();
...
fn test_notification() {
    ...
    use std::time::Duration;
    run_test(ServerOpts::default(), |device| {
        let chan = device.comms.get_notification_channel();
