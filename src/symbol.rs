...
impl Handle {
    ...
    pub fn new(device: Arc<Device>, symbol: &str) -> Result<Self> {
...
pub fn get_size(device: &Device, symbol: &str) -> Result<usize> {
    let mut buf = [0; 12];
...
pub fn get_location(device: &Device, symbol: &str) -> Result<(u32, u32)> {
    let mut buf = [0; 12];
    device.write_read_exact(index::GET_SYMINFO_BYNAME, 0, symbol.as_bytes(), &mut buf)?;
    Ok((
...
pub fn get_symbol_info(device: &Device) -> Result<(Vec<Symbol>, TypeMap)> {
